  ...
  bool getFrameGeneratorConfig(FrameType type, SerializedObject &object)
    {
      ...
      //  FRAME_RAW_FRAME_PROCESSED = 1,
      //  FRAME_DEPTH_FRAME = 2,
        if(type==FRAME_RAW_FRAME_UNPROCESSED)
        {
            return this->_frameGenerators[0]->writeConfiguration(object);
        }
        else if(type==FRAME_RAW_FRAME_PROCESSED)
        {
            return this->_frameGenerators[1]->writeConfiguration(object);
        }
        else if(type==FRAME_DEPTH_FRAME)
        {
            return this->_frameGenerators[2]->writeConfiguration(object);
        }
...
bool DepthCamera::_get(const String &name, T &value, bool refresh) const
{
  ...
  {
    //james mark following line
    //logger(LOG_ERROR) << "DepthCamera: _get Unknown parameter " << _id << "." << name << std::endl;
    return false;
  }
}
...
bool DepthCamera::_set(const String &name, const T &value)
{
  ...
  {
    //james mark following line
    //logger(LOG_ERROR) << "DepthCamera:_set Unknown parameter " << _id << "." << name << std::endl;
    return false;
  }
}
...
const ParameterPtr DepthCamera::getParam(const String &name) const
{
  ...
  {
    //james mark following line
    //logger(LOG_ERROR) << "DepthCamera: getParam Unknown parameter " << _id << "." << name << std::endl;
    return 0;
  }
}
